...
class FrontendLocalizationContractTest(unittest.TestCase):
    ...
    def test_download_entry_is_only_revealed_after_review_authorization(self):
        ...
        self.assertNotIn("Currently open to invited friends only", html)
        self.assertNotIn("Alpha access is shared through invited review links.", html)
        self.assertNotIn("<div data-no-i18n>", html)
        self.assertNotIn("data-review-only data-no-i18n", html)
        self.assertIn("Currently open to invited friends only", localization)
        self.assertIn(
