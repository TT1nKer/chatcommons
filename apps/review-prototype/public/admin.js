(function () {
  ...
  function render(items) {
    ...
    items.forEach((item) => {
      ...
      const statusLabel=node('label','','处理状态'); const select=document.createElement('select'); select.name='status'; [['pending','待确认'],['in_progress','处理中'],['client_review','待朋友验收'],['completed','已完成'],['rejected','暂不处理']].forEach(([value,text])=>{const option=document.createElement('option');option.value=value;option.textContent=text;option.selected=item.status===value;select.appendChild(option);}); statusLabel.appendChild(select);
