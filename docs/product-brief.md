...
Home Server 和备份恢复已经实现。第一版原生协议测试壳和永久测试社区
也已经建立；朋友评审过的网页界面正在成为浏览器与桌面共用的唯一客户端 UI。
账号恢复、多设备和正式发布安全仍未完成。
...
implemented. A native protocol test shell and permanent test community now
exist, while the reviewed web interface is becoming the single client UI shared
by browser review and desktop packaging. Account recovery, multi-device
support, and release hardening are not complete.
